...
document.addEventListener("DOMContentLoaded", () => {
  ...
  filterCategoriesEvent(getServices());
});
...
function makeFilterEvent(services, filterOptions){
  const {tiggerId, targetId} = filterOptions;
  const trigger = document.getElementById(tiggerId);
  const target = document.getElementById(targetId);

  if (!tigger || !target) return;
...
function AllEventFilter(services) {
  ...
  if (!target) return;
  makeFilterEvent(services, {tiggerId: "delai-filter", targetId: target, delai_min: 1, delai_max: 3});
  makeFilterEvent(services, {tiggerId: "note-filter", targetId: "note-detail", rating: 4});
}
